...
def _pre_test(func):
    ...
    def check_pipeline(self, *args, **kwargs):
        ...
        while bad_pipeline and num_test < NUM_TESTS:
            ...
            args = [
                self._toolbox.clone(arg)
                if isinstance(arg, creator.Individual) else arg for arg in args
            ]
            try:
                ...
                for expr_test in expr_tuple:
                    pipeline_code = generate_pipeline_code(
                        expr_to_tree(expr_test, self._pset), self.operators)

                    sklearn_pipeline = eval(pipeline_code,
                ...
                message = '_pre_test decorator: {fname}: num_test={n} {e}.'.format(
                    n=num_test, fname=func.__name__, e=e)
